...
const HumanBody = () => {
  ...
  const handlePieceClick = (event) => {
    ...
    setSelectedArea(event.target.id);
  };
  ...
  return (
    <div>
      <div className="sc-body-model evidence-search-body-widget__body-model">
        ...
        <div
          className="ui-button ui-button--text sc-body-model__rotate absolute-top-right"
          id="rotate"
          ariaHidden="true"
          tabIndex="-1"
      ...
      {selectedArea && (
        <div className="BodyPartsInfo">{bodyPartsInfo[selectedArea]}</div>
      )}
    </div>
  );
};
